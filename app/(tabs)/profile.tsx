...
export default function ProfileScreen() {
  ...
  const posts = useMemo(() => {
    if (!me) return [];
    return allPosts.filter((p) => p.userId === me.id);
  }, [allPosts, me]);
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <AppCard>
            ...
            <Text style={styles.small}>{item.likes.length} likes • {item.comments.length} comments</Text>
          </AppCard>
        )}
      />
    </ScreenContainer>
...
const styles = StyleSheet.create({ header: { gap: spacing.md }, actions: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm }, list: { gap: spacing.sm, paddingBottom: 100 }, caption: { color: colors.text }, meta: { color: colors.secondary, fontWeight: '700' }, small: { color: colors.textMuted } });
